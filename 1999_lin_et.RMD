...
条件独立，其均值为 $E\left(y_{i} | b\right)=\mu_{i}^{b}$ ,方差为 $\operatorname{var}\left(y_{i} | b\right)=\phi m_{i}^{-1} v\left(\mu_{i}^{b}\right)$,
...
O'Sullivan et al.(1986)的结果，注意到方程(2)是 $f_j(\cdot)$ 的一个连续线性函数。
可以表明，对于给定的 $\lambda$ 和 $\theta$ ，使惩罚对数拟似然值最大化的 $f_j(\cdot)$ 
自然立方平滑样条估计器满足：

$$l\left\{y ; \beta_{0}, f_{1}(\cdot), \ldots, f_{p}(\cdot), \theta\right\}-\frac{1}{2} \sum_{j=1}^{p} \lambda_{j} \int_{s_{j}}^{t_{j}} f_{j}^{\prime \prime}(x)^{2} \mathrm{d} x=l\left(y ; \beta_{0}, f_{1}, \ldots, f_{p}, \theta\right)-\frac{1}{2} \sum_{j=1}^{p} \lambda_{j} f_{j}^{\mathrm{T}} K_{j} f$$ {#eq:m3}
...
（(Tierney and Kadane, 1986）。Tierney和Kadane, 1986)。将 [@eq:m3] 的最终近似值最大化，
...
DPQL公式 [@eq:m5] 可以写成(Breslow和Clayton (1993), 式(6), and Zhang等人. (1998),式(11)比较)：
...
使用惩罚拟似然法估计的固定效应 $\hat{\beta}_{j}$ 与式 [@eq:m10] 所示的GLMM模型中
...
由式(11)考察可知，它对应于线性混合模式下$\beta$和$(a,b)$的最佳线性无偏预测量(BLUPs)的
正态方程 ,其线性混合模型为：
...
因此，推导GAMM(1)下DPOL估计值 $\hat {f_j}$ 的贝叶斯标准误差，并将其性能与3.3节末尾计
算的频域标准误差(假设 $f_j(\cdot)$ 为固定光滑函数)进行比较，具有潜在的意义。假设 $f_j$ 
的先验已知,一些计算显示 $(\hat \beta,\hat a)$ 的DPQL估计量，其近似贝叶斯协方差矩阵是：

$$\operatorname{cov}_{\mathrm{B}}(\hat{\beta}, \hat{a})=H^{-1}$$ {#eq:m16}
这比它在式 [@eq:14] 中对应的频域协方差矩阵有更简单的形式。它显示 $\hat{f}_{j}$ 的近似
贝叶斯协方差协方差是
...
(1985)指出 $\tau$ 的这种边际似然([@eq:m15])实际上是线性混合模型下的REML：
...
似然方法，并通过最大化边缘拟似然联合估计 $\tau$ 和 $\theta$ 。具体地说，GLMM表达式
