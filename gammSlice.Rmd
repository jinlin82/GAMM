...
```{r fig1,fig.cap="基于MCMC抽样的简单logistic回归模型的信息总结,dev="png"}
set.seed(39402)
m<-100
n<-2
beta0True<-0.5
...
```{r fig2,fig.cap="有效自由度的MCMC输出信息汇总",dev="png}
set.seed(53902)
n<-400
x<-runif(n)
y<-rpois(n,exp(cos(4*pi*x)+2*x-1))   
...
```{r fig3,fig.cap="贝叶斯后验密度均值和95%的置信区间",dev="png"}
par(mfrow=c(1,2))
#plot(fit2)
plot(fit2,responseScale=TRUE)
points(x,y)
...
```{r fig4,fig.cap="基于切片MCMC抽样的信息总结",dev="png}
set.seed(981127)
n<-500
betax1True<-0.5
x1<-sample(c(0,1),n,replace=TRUE)
...
```{r fig5,fig.cap="f的贝叶斯后验均值估计及95%置信区间",dev="png"}
plot(fit3)
```
...
```{r fig6,fig.cap="基于切片MCMC抽样的模型拟合信息总结",dev="png"}
set.seed(2966703)
m<-100
n<-10
x1<-runif(m*n)
...
```{r fig7,fig.cap="f1和f2的贝叶斯后验均值估计及95%的置信区间",dev="png"}
plot(fit4)
```
...
```{r fig10,fig.cap="基于切片MCMC抽样拟合logistic混合模型的信息汇总",dev="png"}
data("toenail")
terbXmonths<-toenail[,"terb"]*toenail[,"months"]
toenailPlus<-cbind(toenail,terbXmonths)
head(toenailPlus)
fitTN<-gSlc(onycholysis~terb+months+terbXmonths,random=list(idnum=~1),data=toenailPlus,family="binomial")
summary(fitTN)
```
...
```{r fig12,fig.cap="基于切片MCMC抽样的拟合logistic可加混合模型的信息汇总",dev="png"}
data("indonRespir")
head(indonRespir)
fitIR<-gSlc(respirInfec~s(age)+vitAdefic+female+height+stunted+visit2+visit3+visit4+visit5+visit6,
...
```{r fig13,fig.cap="f(age)的贝叶斯后验均值估计及95%的置信区间",dev="png"}
plot(fitIR,responseScale=TRUE)
```
