...
O'Sullivan et al.(1986)的结果，注意到方程 [@eq:m-99-2] 是 $f_j(\cdot)$ 的一个连续线性函数。
...
(Tierney和Kadane, 1986)。将 [@eq:m-99-3] 的最终近似值最大化，
...
通过拟合式 [@eq:m-99-10] 所示的GLMM模型来得到 $\hat{f}_{j}$ 。
...
的后验模态性质。部分积分Wiener先验的设定与式 [@eq:m-99-8] 有相同形式，除了
...
其中 $a \sim N(0, \tau I)$ ，$b$ 和 $\epsilon$ 的分布与 [@eq:m-99-18] 中的相同。
...
数似然(正态)。注意表达式 [@eq:m-99-20] 中的边际对数似然 $l_{M}(y ; \tau, \theta)$ 有一个相似形式
...
其中 $V=B \Lambda B^{\mathrm{T}}+Z D Z^{\mathrm{T}}+W^{-1}$ 。由式 [@eq:m-99-22] 可知，它对应线
...
实现。等价地，只需将工作线性混合模型 [@eq:m-99-12] 迭代拟合到工作向量 $Y$ 上；并使用 $\beta_{j}$ 、 
...
并且，$\hat f_j(x_{ij})$ 是DPQL估计量。注意，修正后的 $\hat{\mu}_{i}^{0}$ 是式 [@eq:m-99-24] 中
...
$$p\left(y | f, \tau^{2}, \beta, b\right)=\prod_{i=1}^{n} L_{i}\left(y_{i} ; \eta_{i}\right)$$ {#eq:m13}
...
完成的。Carlin等人(1992)描述的，单独更新每个参数f(t)的单步移动步骤存在收敛和混合问题。
对于高斯模型，可以采用Gibbs多步采样法。参见Carter and Kohn(1994)和Wong and Kohn(1996)。
对于非高斯响应变量，Gibbs采样不再可行，需要更通用的MH算法。我们采用并扩展了一个计算效
率非常高的MH算法，其中包含Knorr-Held(1999)提出的用于动态广义线性模型的条件先验建议。这
...
第一个因素 $L(fr, s)$ 是式 [@eq:m13] 中所有依赖于 $f[r, s]$ 的似然的乘积。第二个因素， 
...
味着 $K[r, s]^{r}$  和 $K[r, s]$ 的稀疏结构大部分元素是0。例如，对于一阶随机游走，仅
...
为了避免连续块之间的相关性问题，可以采用几种策略;参见Knorr-Held(1999)。在我们的实现中，指定
...
另一种选择是Gamerman(1997)提出的加权最小二乘方案。这里的高斯方案采用均值 $m(\beta)$ 和协
