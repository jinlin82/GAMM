...
年度城市空气出现严重污染的天数和月度城市是否会出现严重污染为因变量建立广义可加混合模型
进行相应研究，为空气质量状况研究提供新的视角和研究方法。
...
很多因素的影响，比如常见的空气质量影响因素包括经济社会因素、地理因素及气象因素。通过查阅
相关文献，结合空气质量的影响因素及数据的可获得性，本文初步选取表中共24个指标进行具体分析。
...
以上指标数据来源于《中国气象年鉴》、《中国统计年鉴》和《城市空气质量状况月报》。对于月度固
定资产投资为累计值，采用后一期减去前一期的计算方法。其他的非月度数据，采用合适的频率转化方
法转化为月度数据以后再进行分析。如经济指标中的月度地区生产总值使用相应的季度生产总值除以3近
似代替。
...
作为比较的基准，利用筛选出的10个变量首先建立泊松回归模型。在10%的显著性水平下，虽然只有降水
...
越差；而降水量越多、临海的地区，年度空气严重污染天数越少，空气质量越好。非参数项的曲线显示在
...
模型中的结果相似，只是纬度对空气出现严重污染的影响由正向作用变为负向作用。在泊松可加混合效
...
Logistic回归模型的结果相似。在10%的显著性水平下，私人汽车拥有量、平均气温、降水量、平均相
...
结果显示在10%的显著性水平下，降水量、平均相对湿度、平均风速、日照时数均通过显著性
...
library("kableExtra")
knitr::kable(month, row.names =T, align = c("r", "r", "r", "r"),
