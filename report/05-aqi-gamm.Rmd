...
# 基于半参数混合模型的城市空气质量影响因素分析 {#sec:chap-5}
...
根据因变量分布的分析，月均AQI取对数后近似服从正态分布。因此，下面以对数月均AQI为
因变量采用月度数据建立线性回归模型、线性混合效应模型、半参数回归模型和半参数混合
效应模型，从这些模型中选择最优模型并做相应分析。
...
中显示了纬度的拟合曲线，阴影部分表示95%的置信带。由图 \@ref(fig:fig-gam-m) 可以
...
```{r fig-gam-m, eval=T, fig.height=2.5, fig.width=6.5,out.width="1.0\\textwidth", fig.pos="H", fig.cap = "月均AQI半参数回归模型非参数曲线", dev=c("cairo_pdf"),dev.args=list(family="Microsoft YaHei UI Light")}
par(mfrow = c(1,1))
par(mar=c(5,4,2,1))
plot.gam(gam.m,
         select=1,
...
```{r fig-gamm-m, eval=T, fig.height=4, fig.width=6.5,out.width="1.0\\textwidth", fig.pos="H", fig.cap = "月均AQI可加混合模型非参数曲线", dev=c("cairo_pdf"),dev.args=list(family="Microsoft YaHei UI Light")}
par(mfrow = c(1,1))
par(mar=c(5,4,2,1))
plot.gam(gamm.m$gam,
         select=1,
...
```{r fig-gam-m-best, eval=T, fig.height=4, fig.width=6.5,out.width="1.0\\textwidth", fig.pos="H", fig.cap = "月均AQI最优模型非参数曲线", dev=c("cairo_pdf"),dev.args=list(family="Microsoft YaHei UI Light")}
par(mfrow = c(1,1))
par(mar=c(5,4,2,1))
plot.gam(gam.m.best,
         select=1,
