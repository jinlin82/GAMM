...
```{r tab-dep-dis, eval=T, results='markup'}
rm(list=ls())
dat.y <- read.csv("./data/yearly.csv")
...
分析空气质量的影响因素。根据因变量的具体分布，且遵循模型由简单到复杂的原则，对服
从正态分布的年均AQI、对数化后的年度空气质量为优的天数及对数化后的月均AQI建立普通
线性回归模型，进一步分别考虑空气质量城市之间的相关性及自变量对空气质量的非线性影
响，分别建立线性混合模型及半参数混合模型；对于服从泊松分布的年度严重污染的天数、
...
变量的取值，纵轴表示年均AQI，数值表示两者之间的简单相关系数。由图
\@ref(fig:fig-aqi-cor) 可知，年均AQI与地区生产总值增长率、建成区绿化覆盖率、城市
公路客运量、平均气温、降水量、平均相对湿度、平均风速、经度、海拔呈负相关关系，与
其他变量呈正相关关系。依据相关程度的大小，在正相关的变量中，相关程度由大到小依次
是：纬度（0.678）、废气污染治理完成投资额（0.383）、私人汽车拥有量（0.317）、固
定资产投资（0.291）、工业烟粉尘排放量（0.249）、工业污染治理完成投资（0.247）、
...
量越好，这四个指标与我们平常的认知有些偏差，也许是因为在这里这种线性关系本身就不
存在的缘故。在图 \@ref(fig:fig-aqi-boxplot) 中，横轴表示是否供暖、是否临海这两个
变量，纵轴表示年均AQI。由图可以直观地看出，供暖的地区比不供暖的地区年均AQI值大，
不临海的地区比临海的地区年均AQI值大。同时可以发现，在供暖的地区存在一个异常值，
在非临海地区也存在一个异常值。进一步地分析可知，22个连续自变量之间，特别是同类影
响因素之间也存在高度线性关系，表明存在多重共线性现象。以上方法只是初步方法，接下
来使用lasso方法进行更加严格的变量筛选。且对于月度数据，并没有给出散点图和相关系
数图，之后直接使用lasso进行回归。
