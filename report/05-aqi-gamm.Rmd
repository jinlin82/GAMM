...
状况只是一个比较笼统的概念。我们除了考虑综合的空气质量指数之外，还需要进一步
考虑空气质量状况的好坏两种极端分类情况：空气质量为优及严重污染。

与国外一些空气质量较好的城市相比，国内大部分城市空气质量为优的天数较少，空气质量
...
两个概率分布的图形化方法，若Q-Q图中绘制的点大致落在直线附近，则样本数据为正态
...
自变量涉及社会经济因素、气象因素、地理因素共24个指标，在回归分析中，由于自变量较
...
```{r fig-aqi-cor, eval=T, fig.height=9, fig.width=6.5,out.width="1.0\\textwidth", fig.pos="H", fig.cap = " 22个自变量与年均AQI散点图", dev=c("cairo_pdf"),dev.args=list(family="Microsoft YaHei UI Light")}
rm(list=ls())
dat.y <- read.csv("./data/yearly.csv")
dat.m <- read.csv("./data/monthly.csv")
...
额、平均气温对年均AQI呈非线性影响。下面就以筛选出的变量建立模型。
...
汽车拥有量、平均气温、降水量、平均风速、平均气压、
是否临海、纬度通过显著性检验，且其系数分别为0.0000、-0.0007、-0.0134、-0.0555、0.0019、0.0005、-0.1965、0.0130。表明平均风速、平均气压、纬度均对对数月均AQI呈正向影响，私人汽车拥有量、
