...
时间序列研究方法以及支持向量机、BP神经网络等算法对未来的空气质量指数进行预测[@刘威2013;闫
妍2013;胡世前2016]。第二类注重从时空演变角度出发注重探究空气质量的时空演变特征[@黄小刚2019;@柏玲2018]。第三类是空气质量影响因素分析。在进行影响因素分析时，因变量设置也有所不同，一类是以SO2等空
...
```{r fig-dep-norm, eval=T, fig.height=2.5,fig.width=6.5,out.width="95%", fig.pos="h", fig.cap = " 空气质量为优天数Q-Q图", dev=c("png","cairo_pdf"),dev.args=list(family="Microsoft YaHei UI Light")}
rm(list=ls())
library(car)
...
qqPlot(dat.y$excellent, id=F, grid = F, xlab = "",ylab='', main ="年度质量为优天数", lwd=1)
qqPlot(dat.m$excellent, id=F, grid = F, xlab = "",ylab='',main ="月度质量为优天数", lwd=1)
```
