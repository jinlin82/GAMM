...
其中 $g(\cdot)$ 是联接函数， $\mu_{i}^{\boldsymbol{b}}$ 是 $y$ 的条件期望，
$\boldsymbol{\beta}$ 是固定效应参数向量， $\boldsymbol{X_{i}}$ 是固定效应设计矩
阵，$f_{j}$ 是自变量 $x_{k}$ 的光滑函数， $\boldsymbol{Z_{i}}$ 是随机效应的设计
矩阵，$\boldsymbol{b \sim \mathcal{N} (0, \Psi)}$ 是随机效应变量，其中方差协方差
阵$\boldsymbol{\Psi}$ 一般未知而需要估计。

在模型中，线性预测子和非参数函数共同来拟合自变量的固定效应，而随机效应用来拟合观
测值之间的相关关系。随机效应 $\boldsymbol{b}$ 的方差协方差矩阵结构需根据数据类型
提前设定。当光滑函数 $f(\cdot)$ 都为线性函数时，那么此时GAMM就变化为GLMM。GAMM的
计算强度较大，Lin，Zhang(1999)使用光滑样条来估计非参数函数，使用边际拟似然同时给
出光滑参数和方差成分的估计[@Lin1999]。本文使用R包mgcv和gamm4进行模型计算。

文章以下部分的结构为：第二部分说明了变量选择和数据来源，给出了数据处理过程；第三
部分在使用Lasso方法选择自变量的基础上，以年度城市空气出现严重污染的天数为因变量
建立了广义可加混合模型的几种具体模型，通过模型比较给出了最优模型并进行解释分析得
到影响因素。第四部分以月度城市是否会出现严重污染为因变量作了类似的分析得到月度空
气质量因素因素。最后部分是结论。
...
根据表 \@ref(tab:tab-severe-m-huizong) 可知，在Logistic回归模型(模型I)中，显著的
变量有常住人口、私人汽车拥有量、平均气温、降水量、平均相对湿度、平均风速、日照时
数、纬度；在Logistic混合效应模型(模型II)中，显著的变量有私人汽车拥有量、平均气温、
降水量、平均相对湿度、平均风速、日照时数、纬度；在Logistic可加模型(模型III)中，
显著的变量有降水量、平均相对湿度、平均风速、日照时数、常住人口、平均气温、纬度；
在Logistic可加混合效应模型(模型IV)中，显著的变量有降水量、平均相对湿度、平均风速、
日照时数。通过比较以上模型的AIC值和模型的实际意义，本文选择Logistic可加模型(模型
III)作为最优模型，其AIC值最小，为782.7841。但在上面估计的Logistic可加模型(模型
III)中，是否供暖和是否临海等自变量未通过显著性检验，因此将其删除并重新建立
Logistic回归模型。
