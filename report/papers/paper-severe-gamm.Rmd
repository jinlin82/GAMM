...
随着我国经济的快速发展，人民生活条件得到大大的改善，但也随之带来了严重的
环境污染问题，引起人们的广泛关注。为此，我国制定了一系列环境空气质量的政策。2018年6月
27日，国务院印发关于打赢蓝天保卫战三年行动计划的通知，计划明确表示，到2020年,要求二氧化
硫、氮氧化物排放总量相比2015年分别减少至少15%；其中，PM2.5未达标的地级及以上城市，PM2.5
浓度要比2015年下降18%以上，地级及以上城市重度及以上污染天数比率比2015年下降25%以上。环境
空气质量的改善离不开系统的管理决策以及有效的科学理论支持。除了制定相关政策以外，对环境空
气质量进行科学研究也是必不可少的，空气污染影响因素研究便是其中的一项重要内容。故本文重
点关注空气污染天数，以年度空气严重污染天数和月度空气严重污染天数为研究对象，在利用Lasso方法
筛选影响因素的基础上，基于广义可加混合模型对空气严重污染天数进行影响因素探究，既可完善相关科学
理论，也可为有关环境治理措施的制定及实施提供借鉴意义。

在进行空气质量污染研究时，学者们根据使用的方法得出不同的结论。部分学者利用空间分析方法得出
不同时空特征下，空气质量污染的影响因素存在地理和季节上的差异[@陈卫卫2019;@李惠娟2018],影响
因素的影响方向和影响程度也随时间变化[@郭一鸣2019]。部分学者以大气污染物代表空气污染作为研究
对象，利用灰色关联分析法探讨空气污染影响因素[@王超2019;@王乐2019]。罗静利用负二项回归的方法
得出人口因素、气象因素以及季节效应的发生等因素对空气质量污染程度有显著影响[@罗静2017]。另外，
学者们利用环境库兹涅茨曲线模型、面板分析方法、结构方程模型等方法研究城市化进程、产业结构、人
口因素等社会经济因素对空气质量污染的影响[@李健2019;@李斌2014;王立平2016；李静萍2017；@吴雪萍2018]。

基于以上文献分析，可以发现：基于以上文献分析，可以发现：第一、在考虑空气质量的影响因
素时，鲜有研究引入地理因素。第二，在分析空气质量影响因素时，影响因素对空气质量的影响
存在时空上的变化，但当前使用的方法很少有同时考虑影响因素对空气质量的线性效应和非线性
效应,且将不同城市间影响因素对于空气质量的影响设置为随机效应,
也即城市间可能存在随机效应。因此，本文选取2014-2017年中国重点监测的35个城市的年度数据
和月度数据，综合考虑社会经济因素，气象因素和地理因素三方面的影响，分别以年度城市空气出
现严重污染的天数和月度城市是否会出现严重污染为因变量建立广义可加混合模型进行相应研究，
为空气质量状况研究提供新的视角和研究方法。
...
天数进行一系列正态性检验，图\@ref(fig:fig-dep-dist) 显示两个变量均不服从正态分布。故根据数据特征，将年度严重污染的
...
```{r fig-dep-dist, eval=T, fig.height=7,fig.width=6.5,out.width="1.0\\textwidth", fig.pos="H", fig.cap = "空气质量严重污染天数 Q-Q 图", dev=c("cairo_pdf"),dev.args=list(family="Microsoft YaHei UI Light")}
rm(list=ls())
...
空气质量差异较大。下面就以筛选出的变量和其余两个分类型变量共10个变量建立模型。

```{r tab-geo, eval=FALSE, include=FALSE, results='markup'}
varlist <- read.csv("../data/var_list.csv")
dat.y <- read.csv("../data/yearly.csv")
...
month..[seq(1,41,4)+1,(2:5)]=apply(month..[seq(1,41,4)+1,(2:5)],2,function(x) {paste('(',x,')',sep='')})
...
回归模型(模型I)作为最优模型，其AIC值最小，为787.2738。但在上面估计的Logistic回归模型(模型I)中，
是否供暖和是否临海两个自变量未通过显著性检验，因此将其删除并重新建立Logistic回归模型。

以月度是否出现严重污染为因变量，常住人口、私人汽车拥有量、平均气温、降水量、平均相对湿度、平
均风速、日照时数、纬度为自变量，建立Logistic回归模型，模型估计结果中
...
varselname <- as.character(varselname[order(varselname$var_no), 2])
library(mgcv)
gam.m.severe.best <- glm(severe~pop+car
                   +mean_temp+rain+humid+wind_speed+sun
                   +lat,
                   data=dat.sel.m.severe,
                   family = binomial)

gam.m.severe.best.coef <- summary(gam.m.severe.best)$coef

dimnames(gam.m.severe.best.coef) <- list(c("截距项", varselname[-c(8,9)]), c("估计值", "标准误", "t值", "P值"))
...
由表 \@ref(tab:tab-severe-gam-m-best) 可知，在10%的显著性水平下，常住人口、私人汽车拥有量，平均气温、
降水量、平均相对湿度、平均风速、日照时数、维度均通过显著性检验，表明在
其他条件不变的情况下，私人汽车拥有量越大，纬度越高，月度出现严重污染的概率越大，常住人口、平均气温、降水量越大、平均相对湿度越大、平均风速
...
模型结果中私人汽车拥有量对月度严重污染的概率造成正向影响也符合实际情况。
...
渐增加。气候因素中的降水量、平均风速、日照时数对年度空气严重污染天数有显著
影响，并且降水量是负的线性影响，平均风速和日照时数是非线性影响。地理因素中仅有纬度对年度空气严
重污染天数有显著性线性影响。

对于月度是否出现空气严重污染，社会经济影响因素有常住
人口数、私人汽车拥有量，其与月度是否出现空气严重污染概率之间具有线性关系；气候
因素中的平均气温，降水量，平均相对湿度，平均风速和日照时数对月度是否出现空气严重
污染的概率局有显著性影响，这5个气候因素是负向线性影
响。地理因素中只有纬度变量对月度是否出现空气严重污染有显著性影响，并且这个影响是
线性的。通过比较年度空气严重污染天数和月度是否出现空气严重污染之间的影响因素，
我们发现二者并不完成重合。

# 参考文献 {-}
[//]: # (\bibliography{Bibfile_s})
