...
度空气质量严重污染天数为研究对象，利用广义可加混合模型对城市空气质量的影响因素进
行研究。结果显示，对于年度空气严重污染天数，社会经济影响因素中的建成区绿化覆盖率
是影响天数多少的主要因素，并且这个影响是非线性的，气候因素中的降水量、平均风速、
日照时数对年度空气严重污染天数有显著影响，并且降水量是负的线性影响，平均风速和日
照时数是非线性影响。地理因素中仅有纬度对年度空气严重污染天数有显著性线性影响。对
于月度是否出现空气严重污染，社会经济影响因素主要有常住人口数，其与因变量之间具有
非线性关系；气候因素中的平均气温，降水量，平均相对湿度，平均风速和日照时数对月度
是否出现空气严重污染的概率局有显著性影响，这5个气候因素中，除平均温度是负的非线
性影响外，其余是负向线性影响。地理因素中只有纬度变量对月度是否出现空气严重污染有
显著的线性影响。
...
cities monitored in China from 2014 to 2017 as the research object, and uses the
generalized additive mixed model to study the influencing factors of urban air
quality. The results show that for the yearly severe air pollution days, the
greening rate in the socio-economic factors has a nonlinear effect;
precipitation,average wind speed and sunshine hours in the climate factors have
significant effects. Among the geographical factors, only the latitude has a
significant influence on the annual severe air pollution days, and the
relationship is nonlinear. For the monthly severe air pollution days, the factor
...
根据表 \@ref(tab:tab-severe-y-huizong) 结果可知，在泊松回归模型(模型I)中，显著的
变量有常住人口、建成区绿化覆盖率、私人汽车拥有量、平均相对湿度、平均风速、日照时
数、供暖、临海；在泊松混合效应模型(模型II)中，显著的变量有常住人口、降水量、平均
风速、日照时数、纬度；在泊松可加模型(模型III)中，显著的变量有降水量、供暖、临海、
纬度、建成区绿化覆盖率、平均风速、日照时数；在泊松可加混合效应模型(模型IV)中，显
著的变量有降水量、临海、纬度、建成区绿化覆盖率、平均风速、日照时数。从模型拟合优
度来看，泊松混合效应模型(模型II)效果最好，其AIC值最小，为534.3405；泊松可加混合
效应模型(模型IV)次之，其AIC为540.6188。但泊松混合效应模型(模型II)中是否临海这个
地理因素不显著，而根据第二部分的t检验可知，临海和不临海的城市的空气质量之间存在
显著差异。故综合考虑模型拟合优度和之前的变量筛选结果，将泊松可加混合效应模型(模
型IV)确定为年度空气严重污染天数的最优模型。接下来去掉泊松可加混合效应模型(模型
IV)中不显著的变量，重新构建新的模型。

以年度空气严重污染天数为因变量，降水量、临海、纬度、建成区绿化覆盖率、平均风速、
日照时数为自变量建立泊松可加混合效应模型，模型估计的参数部分结果显示在表
\@ref(tab:tab-severe-gam-y-best) 中,非参数项的曲线显示在图
\@ref(fig:fig-severe-gamm-y-best) 中,非参数项显著性检验结果在表
...
library(mgcv)
gamm.y.severe.best <- gamm4(severe~green
...
gamm.y.severe.best.coef <- summary(gamm.y.severe.best$mer)$tTable[1:4,-3]
...
```{r fig-severe-gamm-y-best, eval=T, fig.height=5.5, fig.width=6.5,out.width="95%", fig.pos="H", fig.cap = "年度空气严重污染天数泊松可加混合模型非参数曲线", dev=c("png","cairo_pdf"),dev.args=list(family="Microsoft YaHei UI Light")}
par(mfrow = c(2,2))
par(mar=c(5,4,2,1))
...
现空气严重污染的非参数关系曲线，且从表 \@ref(tab:tab-severe-gam-m-smooth-best)中
可以看出，在10%的显著性水平下，三个变量对月度是否出现严重污染的概率的非线性影响
均是显著的。由图 \@ref(fig:fig-severe-gam-m-best) 可以看出，月度是否出现空气严重
污染的概率一开始随着常住人口数的上升而缓慢上升，但对于常住人口数超过2000万的城市
而言，月度出现空气严重污染的概率会下降。对于城市月平均温度，月度出现空气严重污染
的概率随着平均温度的上升而下降，但这个下降不是线性的。对于纬度，月度出现空气严重
污染的概率随着纬度的上升而上升，但到了北纬40度之后，月度出现空气严重污染的概率不
再随着纬度上升而上升，而是基本持平的。
