...
降水量、供暖、临海、纬度、建成区绿化覆盖率、平均风速、日照时数；在泊松可加混合效应模型中，
显著的变量有降水量、临海、纬度、常住人口、建成区绿化覆盖率、平均风速、日照时数。从模型拟合
优度来看，泊松混合效应模型效果最好，其AIC值最小，为534.3405。接下来去掉泊松混合效应中不显著的
变量，重新构建新的模型。

以年度空气严重污染天数为因变量，常住人口、降水量、平均风速、日照时数、纬度为自变量建立泊松混合
效应模型，模型估计的参数部分结果显示在表
\@ref(tab:tab-excel-gam-y-best) 中。


```{r tab-severe-gam-y-best, eval=T, results='markup'}

library(mgcv)
gam.y.severe.best <- gamm(severe~pop
                    +rain+wind_speed+sun
                   +lat,
                   random=list(city=~1), data=dat.sel.y.severe,
                   family="poisson", method = "REML")
...
gam.y.severe.best.coef <- summary(gam.y.severe.best$lme)$tTable[,-3]

dimnames(gam.y.severe.best.coef) <- list(c("截距项", varselname[c(1,4,6,7,10)]), c("估计值", "标准误", "t值", "P值"))
...
速、日照时数、纬度均通过显著性检验，且参数项系数分别为-0.0012、-0.9639、
-0.0010、0.2252。表明在其他条件不变的情况下，纬度越高的地区，年度空气严
重污染天数越多，空气质量越差。而降水量越多、平
均风速越大、日照时间越长，年度空气严重污染天数越少，空气质量越好。
...
回归模型作为最优模型，其AIC值最小，为787.2738。但在上面估计的Logistic回归模型中，
...
```{r fig-severe-gam-m-best, eval=FALSE, fig.cap="月度是否出现空气严重污染最优模型非参数曲线", fig.height=5.5, fig.pos="H", fig.width=6.5, dev=c("png","cairo_pdf"), dev.args=list(family="Microsoft YaHei UI Light"), include=FALSE, list(family="Microsoft, out.width="95%"}
par(mfrow = c(2,2))
par(mar=c(5,4,2,1))
...
```{r tab-severe-gam-m-smooth-best, eval=FALSE, include=FALSE, results='markup'}
