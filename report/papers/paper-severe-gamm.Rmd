...
**Key Words:** Number of Severe Air Pollution Days; Generalized Additive Mixed Model; Influencing Factors
...
<!--                    family = "poisson", method = "REML") -->
<!-- AIC(gam.y.severe$lme);BIC(gam.y.severe$lme) -->
<!-- #summary(gam.y.severe) -->
<!-- #gam与gamm的运算结果系数略有差异，最终结果以gamm为主 -->
...
month=as.data.frame(cbind(rowname,month..))
...
由表 \@ref(tab:tab-severe-gam-m-best) 可知，在10%的显著性水平下，参数部分的降水量、平均相对湿度、
平均风速、日照时数、均通过显著性检验，表明在
...
对于年度空气严重污染天数，社会经济影响因素中的建成区绿化覆盖率是影响天数多少的主要因素，
并且这个影响是非线性的，总体而言，年度空气严重污染天数是随着建成区绿化覆盖率的增加而逐
渐增加，这很可能是是因为先污染后治理模式的存在。人们预先意识不到污染的严重性，只是在空
气质量变差以后才加强环境治理，于是造成空气质量污染越严重，绿化率越高的情况。气候因素中
的降水量、平均风速、日照时数对年度空气严重污染天数有显著影响，并且降水量是负的线性影响，
平均风速和日照时数是非线性影响。地理因素中仅有纬度对年度空气严重污染天数有显著性线性影响。

对于月度是否出现空气严重污染，社会经济影响因素主要有常住人口数，其与月度是否出现空气严重
污染概率之间具有非线性关系；气候因素中的平均气温，降水量，平均相对湿度，平均风速和日照时
数对月度是否出现空气严重污染的概率局有显著性影响，这5个气候因素中，除平均温度是负的非线
性影响外，其余是负向线性影响。地理因素中只有纬度变量对月度是否出现空气严重污染有显著性影
响，并且这个影响是线性的。通过比较年度空气严重污染天数和月度是否出现空气严重污染之间的影
响因素，我们发现二者并不完成重合。这可能是由于年度空气质量严重污染天数相比月度空气质量严
重污染天数更具稳定性。
