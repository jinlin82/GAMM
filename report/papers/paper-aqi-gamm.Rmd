...
- [ ] 半参数可加混合模型简介
- [ ] 两个汇总系数表中模型I，II，III，IV 对应的具体模型
- [ ] 两个汇总系数表下面注明显著性符号
# 摘要

本文以2014-2017年中国重点监测的35个城市的年度和月度空气质量指数为研究对象，利用半参数
混合效应模型对城市空气质量的影响因素进行研究。该模型综合考虑了自变量对因变量的线性和非
线性固定效应以及描述观测值数据相关性的随机效应。研究结果显示，对于年均AQI，社会经济因素
中的废气治理完成投资额是正的线性影响，固定资产投资和汽车拥有量是非线性关系；气候因素中
的平均风速是线性关系，平均气温具有非线性关系;是否临海，是否供暖和纬度三个地理
因素对年均AQI有线性影响。对于月均AQI，社会经济方面的三个影响因素废气治理完成投资额，房屋建
筑施工面积和汽车拥有量均为线性影响；平均气温，降水量，平均风速和平均气
压这四个气象变量对月均AQI均有显著的线性影响；地理因素中的是否临海，是否供暖是线性影响，
纬度对月均AQI有显著的非线性影响。

# 简介

近几年来，空气质量问题引起了社会各界的广泛关注，城市空气污染已经成为了一个严重的
社会问题。恶劣的空气环境不只严重影响了人们的健康和正常生活，也给社会各行各业带来
了巨大的经济损失。日益下降的空气质量状况警醒我们比须及时采取相关措施提升城市空气
质量。在2018年召开的全国生态环境保护大会上，习近平总书记就提出要加大力度推进生态
文明建设、解决生态环境问题，坚决打好污染防治攻坚战，并要求坚决打赢蓝天保卫战是重
中之重，要以空气质量明显改善为刚性要求，基本消除重污染天气，还老百姓蓝天白云。要
做到这些，就需要找到空气质量的影响因素，进而根据影响因素及影响程度的大小进行相应
的处理。
...
在随机效应。因此，本文以空气质量指数为研究对象，利用半参数混合效应模型对城市空气
质量的影响因素进行研究。本文选取2014-2017年中国重点监测的35个城市的年度数据和月
度数据，综合考虑社会经济因素，气象因素和地理因素三方面的影响，分别以年度平均AQI
和月度平均对数AQI为因变量建立半参数混合效应模型进行相应研究，为空气质量状况研究
提供新的研究方法，并完善空气质量状况的研究体系。

**半参数混合效应模型简介**

在实际建模过程中自变量与因变量的关系并不都是非线性关系，最具灵活性的模型应是在同
一个模型中既包含参数项又包含非参数项，模型的参数部分还允许像哑变量或定序变量的离散
变量与参数项一起建模，当包含离散变量，便可以研究分组数据。但经典的线性假设认为观测
值来自一个独立的总体，即是独立同分布的，对于一般的分组数据，其组内观测值是相关的，
因为他们属于相同的子总体，而组间是独立的。为此，综合考虑混合效应使用半参数混合
效应模型，该模型中的混合效应是分析分组数据的灵活工具，纵向数据、重复观测数据、多水
平数据、层次数据等都属于分组数据，混合效应通过复杂的方差结构可以对分组数据中存在的
组内相关进行建模，同时，还可以拟合异方差数据。一般地，半参数混合模型具有以下形式
...
在模型中，线性预测子和非参数函数共同来拟合自变量的固定效应，允许自变量与因变量的关系
存在多种形式，而随机效应用来拟合观测值之间的相关关系和数据的异质性。
...
素，本文采用 $t$ 检验探究城市空气质量指数与地理环境之间的关系。对2017年35个城市的相
关数据进行 $t$ 检验，结果显示在本文所研究的35所城市中，集中供暖的有17个，剩下的18所
城市不采取集中供暖。在集中供暖的城市中，2017年年均AQI为99.79。而在没有集中供暖的
18所城市中，其年均空气质量指数为72.94。且在5%的显著性检验下，检验结果通过显著性
检验，说明集中供暖的城市与没有集中供暖的城市其空气质量指数存在显著差异。同样，在
5%的显著性水平下，临海和不临海的城市的空气质量指数之间也存在显著差异。在本文研究
范围内，临海城市有10个，内陆城市有25个，临海城市年均AQI为71.21。内陆城市年均AQI
为91.89。以上分析表明，城市空气质量与地理因素有关，是否集中供暖和是否临海其城市
的空气质量差异较大。因此，这两个变量也作为影响因素参与模型构建。
...
些可选模型中通过综合比较模型拟合优度，模型实际意义等确定最优模型。
...
我们选取年均AQI作为因变量，基于Lasso筛选出的变量为自变量，分别建立线
性回归模型、线性混合效应模型、半参数回归模型和半参数混合效应模型对空气质量指数的
影响因素进行分析，模型结果显示在表  \@ref(tab:tab-aqi-y-huizong) 中。
...
我们选取对数月均AQI作为因变量，基于Lasso筛选出的变量为自变量，分别建立线性回归模
型、线性混合效应模型、半参数回归模型和半参数混合效应模型对空气质量的影响因素进行
分析。模型结果在表 \@ref(tab:tab-aqi-m-huizong) 中。
...
是否临海均通过显著性检验，且其系数分别为0.0000、0.0003、0.0959、-0.0140、-0.0677、
