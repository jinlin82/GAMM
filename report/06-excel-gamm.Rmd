...
# 基于广义可加混合模型的空气质量为优天数影响因素分析 {#sec:chap-6}
...
划分标准，就可以计算得到2014年-2017年35城市的年度空气质量为优天数。根据第
...
knitr::kable(gamm.y.excel.coef, row.names =T, align = c("r", "r", "r", "r"),
             caption="年度空气质量为优天数半参数混合效应模型回归固定效应表",digits = 4,
             longtable = TRUE, booktabs = TRUE, escape = F) %>%
    kable_styling(latex_options = c("striped", "scale_down", "repeat_header", "hold_position"),
                  ## stripe_index = rep(1:5, 4)+rep(c(0,10,20,30), each = 5),
                  repeat_header_text = "(续)")%>%
...
区生产总值的增加而增加，在1000-2000亿元之间，年均空气质量为优的天数随地区生产总
...
轴表示各自变量，纵轴表示标准化后的月均空气质量为优的天数。
由图 \@ref(fig:fig-var-sel-m-excel) 可知，地区生产总值、地区生产总值增长率、
固定资产投资额、常住人口、建成区绿化覆盖率、城市公路客运量、私人汽车拥有量、全社
会用电量、工业二氧化硫排放量、工业烟粉尘排放量、工业污染治理完成投资额、平均气压、
日照时数、经度、海拔对月均空气质量为优的天数无影响，这些变量被剔除，而剩下的变量
有房屋建筑施工面积、废气污染治理完成投资额、平均气温、降水量、平均相对湿度、平均
风速、纬度。且在剩下的这些变量中，可以看出，平均气温、降水量、平均相对湿度、平均
风速对月均空气质量为优的天数呈正向影响，而房屋建筑施工面积、废气治理完成投资对月
均空气质量为优的天数呈负向影响，纬度与月均空气质量为优的天数呈非线性相关关系。
...
0.031、0.0038、0.0309、0.3306、0.1992、0.0826、-0.017，说明平均气温、降水量、平
...
地表明变量与年均空气质量为优天数之间的关系。综合考虑模型的拟合优度及模型的实际意
